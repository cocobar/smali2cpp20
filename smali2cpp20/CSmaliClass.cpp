...
void CSmaliClass::dumpToTypeHHH(CodeDumper* d) {
	std::string strSelfHName = this->getClassCppSaveName();

	//if (listTemplateParam.size() > 0) {
	//	d->add("#include \"").add(strSelfHName).add(".h").add("\"").newline();
	//	return;
	//}
	...
	hMethodDumper.add("class ").add(strClassName);
	...
	d->add("// Type ").add(this->strFilePath).newline();
	d->add(hMethodDumper);
	d->newline();
...
#if 0
// 只有一个类申明
void CSmaliClass::dumpToType(CodeDumper* d) {
		...
		if (strSelfHName != strInc) {  // 不是自己类的
			...
			if (strInc == strInc2) {
				...
				if (((int)strInc3.find("<")) > 0) {
					...
					d->add("#include \"").add(strInc).add(".hh\"").newline();
				}
			}
			else {
				bool bFind = false;
				...
				if (!bFind) {
					d->add("#include \"").add(strInc).add(".hhh\"").newline();
				}
			}
		}
	}
